...
async def get_list_movies(db: AsyncSession, per_page: int, offset: int) -> list[models.MovieModel]:
    stmt = select(models.MovieModel).offset(offset).limit(per_page)
    result = await db.scalars(stmt)
    return result.all()

async def get_single_movie(db: AsyncSession, movie_id: int) -> models.MovieModel | None:
